...
> [!NOTE]
> The database setup script is provided under `"scripts"` in `package.json` as `"db:setup"` for setting up the database schema. A `.env` file containing the database connection URL defined as `DATABASE_URL` is required for the script to run successfully.
